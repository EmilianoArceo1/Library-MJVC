...
export default function Home(){
  ...
  return <main>
    ...
    {currentUser&&currentUser.approvalStatus!=="approved"&&<div className={`account-status-banner ${currentUser.approvalStatus}`}><div><b>{currentUser.approvalStatus==="pending"?"Tu registro está pendiente de aprobación":"Tu registro no está aprobado"}</b><span>{currentUser.approvalStatus==="pending"?"Un administrador o asesor debe revisar tu solicitud antes de que puedas tomar libros, publicar o proponer títulos.":"Revisa la decisión y cualquier mensaje que te hayan dejado."}</span></div><button onClick={()=>setView("notificaciones")}>Ver notificaciones{notificationUnread>0?` (${notificationUnread})`:""}</button></div>}
