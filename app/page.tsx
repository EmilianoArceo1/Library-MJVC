...
export default function Home(){
  ...
  const readBooks=books.filter(book=>book.rightsStatus!=="rights_reserved"||book.markedRead);
  ...
  const [uploadRightsStatus,setUploadRightsStatus]=useState<string>("review"),[manualPages,setManualPages]=useState("");
  ...
  const [inspectionBook,setInspectionBook]=useState<Book|null>(null),[reservedReadBusy,setReservedReadBusy]=useState(false);
  ...
  const loggedIn=Boolean(currentUser),currentName=currentUser?.name||"Lector",isApproved=currentUser?.emailVerified===true&&currentUser?.approvalStatus==="approved",isModerator=currentUser?.role==="admin"||currentUser?.role==="advisor";
  useEffect(()=>{
    let cancelled=false;
    fetch("/api/auth/me")
      .then(async response=>{
  ...
  useEffect(()=>{
    ...
    if(view==="gestion"&&(!isApproved||!isModerator)){
      setView(currentUser.emailVerified?"biblioteca":"notificaciones");
      return;
    }
    if(view==="subir"&&!isApproved)setView("notificaciones");
  },[currentUser?.approvalStatus,currentUser?.emailVerified,currentUser?.role,view,isApproved,isModerator]);
  useEffect(()=>{
    const url=new URL(window.location.href);
    const result=url.searchParams.get("emailVerification");
    if(!result)return;
    if(result==="success")flash("Correo verificado. Tu solicitud de registro ya fue enviada para revisión.");
    else if(result==="expired")flash("El enlace de verificación venció. Puedes pedir uno nuevo desde tu cuenta.");
    else flash("El enlace de verificación no es válido o ya no está disponible.");
    url.searchParams.delete("emailVerification");
    window.history.replaceState({},"",url.pathname+url.search+url.hash);
    fetch("/api/auth/me",{cache:"no-store"})
      .then(response=>response.json())
      .then(payload=>{if(payload?.user)setCurrentUser(payload.user)})
      .catch(()=>undefined);
  },[]);
  const resendVerification=async()=>{
    if(!currentUser||currentUser.emailVerified||verificationResending)return;
    setVerificationResending(true);
    try{
      const response=await fetch("/api/auth/resend-verification",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:currentUser.email})});
      const payload=await response.json();
      if(!response.ok)throw new Error(payload.error||"No se pudo reenviar el correo");
      flash(payload.message||"Revisa tu correo para continuar.");
    }catch(error){
      flash(error instanceof Error?error.message:"No se pudo reenviar el correo.");
    }finally{
  ...
  const handleBookFile=async(file?:File)=>{
    ...
    try{
      ...
      setManualPages(String(reconstructed.pages.length));
  ...
  const submitBook=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    const rightsStatus=String(data.get("rightsStatus")||"review");
    const isReserved=rightsStatus==="rights_reserved";
    const hasFile=file instanceof File&&file.size>0;
    if(!isReserved&&!hasFile){flash("Selecciona el archivo del libro.");return}
    if(hasFile&&(detectedPages<1||!bookPackage)){flash("Espera a que terminemos de reconstruir el documento.");return}
    const pages=hasFile?detectedPages:Number(data.get("pages"));
    if(!Number.isInteger(pages)||pages<1){flash("Indica el número de páginas de la obra.");return}
    data.set("pages",String(pages));
    if(hasFile)data.set("contentPackage",bookPackage);
    else data.delete("contentPackage");
  ...
  const submitAuth=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    try{
      ...
      setAuthView(null);
      if(authView==="signup"){
        flash(payload.emailVerificationPending?"Cuenta creada. Revisa tu correo y abre el enlace para enviar después tu solicitud de aprobación.":payload.pending?"Tu solicitud fue enviada al administrador para comprobar y aceptar el registro.":payload.user?.role==="admin"?"Cuenta de administrador configurada.":"Cuenta creada correctamente.");
      }else if(payload.user?.emailVerified===false){
        flash("Tu correo todavía no está verificado. Revisa tu bandeja o pide un nuevo enlace.");
  ...
  return <main>
    <header className="topbar"><button className="brand" onClick={()=>setView("biblioteca")}><BookLogo/><span><b>Biblioteca Jornadas</b><small>Biblioteca virtual MJVC Mérida · colección, préstamo y lectura</small></span></button><nav>{[["biblioteca","Estantería"],["comunidad","Lectores"],["foro","Foro"]].map(([id,label])=><button key={id} className={view===id?"active":""} onClick={()=>setView(id as View)}>{label}</button>)}</nav><div className="profile-menu"><button type="button" className="theme-switch" role="switch" aria-checked={theme==="dark"} aria-label={theme==="dark"?"Cambiar a modo claro":"Cambiar a modo oscuro"} title={theme==="dark"?"Modo oscuro activado":"Modo claro activado"} onClick={toggleTheme} disabled={themeSaving}><span className="theme-switch-track" aria-hidden="true"><i/></span><span className="theme-switch-label">{theme==="dark"?"Oscuro":"Claro"}</span></button>{authLoading?<span className="login-link">Cargando…</span>:loggedIn?<><button className={`notification-link ${view==="notificaciones"?"active":""}`} onClick={()=>setView("notificaciones")} aria-label="Notificaciones"><span aria-hidden="true">●</span><b>Notificaciones</b>{notificationUnread>0&&<i>{notificationUnread>99?"99+":notificationUnread}</i>}</button>{isModerator&&isApproved&&<button className={`management-link ${view==="gestion"?"active":""}`} onClick={()=>setView("gestion")}>Gestión</button>}{isApproved&&<button className="upload-link" onClick={()=>setView("subir")}>{isModerator?"＋ Subir libro":"＋ Proponer libro"}</button>}<button className="me" onClick={()=>setModal("profile")}><Avatar name={currentName} color="#cf915f" small src={currentUser?.photoUrl}/><span>{currentName}{currentUser?.role==="admin"?" · Admin":currentUser?.role==="advisor"?" · Asesor":""}</span></button><button className="logout" onClick={logout}>Cerrar sesión</button></>:<><button className="login-link" onClick={()=>setAuthView("login")}>Iniciar sesión</button><button className="upload-link" onClick={()=>setAuthView("signup")}>Registrarse</button></>}</div></header>
    {currentUser&&(!currentUser.emailVerified||currentUser.approvalStatus!=="approved")&&<div className={`account-status-banner ${!currentUser.emailVerified?"email-pending":currentUser.approvalStatus}`}><div><b>{!currentUser.emailVerified?"Verifica tu correo":currentUser.approvalStatus==="pending"?"Tu registro está pendiente de aprobación":"Tu registro no está aprobado"}</b><span>{!currentUser.emailVerified?"Aún no hemos enviado tu solicitud al administrador ni a los asesores. Abre el enlace que llegó a tu correo para continuar.":currentUser.approvalStatus==="pending"?"Tu correo ya fue confirmado. Ahora un administrador o asesor debe revisar tu solicitud antes de que puedas tomar libros, publicar o proponer títulos.":"Revisa la decisión y cualquier mensaje que te hayan dejado."}</span></div>{!currentUser.emailVerified?<button onClick={resendVerification} disabled={verificationResending}>{verificationResending?"Enviando…":"Reenviar correo"}</button>:<button onClick={()=>setView("notificaciones")}>Ver notificaciones{notificationUnread>0?` (${notificationUnread})`:""}</button>}</div>}

    {view==="biblioteca"&&<div className="library-layout">
      <aside className="left-panel"><p className="eyebrow">EN TU MESITA</p><h2>{owned?"Una historia te espera":"Tu mesita está libre"}</h2>{owned?<><button className="owned-cover" style={{background:owned.cover}} onClick={()=>setView("lector")}><span>{owned.title}</span><small>{owned.author}</small><i>{owned.progress??0}%</i></button><div className="progress"><span style={{width:`${owned.progress??0}%`}}/></div><p className="muted">{owned.countsForStats===false?`Avance ${owned.progress??0}% · esta lectura no suma páginas`:(owned.progress??0)<=0?"Portada":`Avance ${owned.progress}% · ${owned.pages} páginas`}</p><div className="pair"><button className="primary" onClick={()=>setView("lector")}>Continuar</button><button className="secondary" onClick={openReturnModal}>Devolver</button></div></>:<p className="empty-note">Explora el estante y elige tu próxima lectura.</p>}<div className="leader-mini"><div className="section-title"><div><p className="eyebrow">ZONA DE LECTORES</p><h3>Quienes más han leído</h3></div><button onClick={()=>setView("comunidad")}>Ver todos →</button></div><div className="avatar-row">{people.map((p,i)=><button key={p.name} onClick={()=>setView("comunidad")}><span className="rank">{i+1}</span><Avatar name={p.name} color={p.color} small src={p.photoUrl}/></button>)}</div></div><button className="forum-card" onClick={()=>setView("foro")}><span>Conversaciones del club</span><b>Entrar al foro <i>↗</i></b></button></aside>
      <section className="shelf-area"><div className="welcome"><div><p className="eyebrow">{loggedIn?`HOLA, ${currentName.toUpperCase()}`:"CATÁLOGO MJVC MÉRIDA"}</p><h1>¿Qué historia te llama hoy?</h1></div><p>{filtered.length} títulos en el estante</p></div><div className="filters"><label className="search"><span>⌕</span><input value={search} onChange={e=>{setSearch(e.target.value);setShelfPage(0)}} placeholder="Busca por título o autor"/></label><label><span>Año</span><select value={year} onChange={e=>{setYear(e.target.value);setShelfPage(0)}}><option value="">Todos</option>{[...new Set(books.map(b=>b.year))].sort((a,b)=>b-a).map(y=><option key={y}>{y}</option>)}</select></label><label><span>Tipo</span><select value={type} onChange={e=>{setType(e.target.value);setShelfPage(0)}}><option value="">Todos</option>{[...new Set(books.map(b=>b.type))].map(t=><option key={t}>{t}</option>)}</select></label>{(search||year||type)&&<button className="clear" onClick={()=>{setSearch("");setYear("");setType("");setShelfPage(0)}}>Limpiar</button>}</div><div className="shelf-card"><div className="shelf-head"><span>COLECCIÓN GENERAL · A–Z</span><span>Estante {shelfPage+1} de {shelfCount}</span></div><div className="books">{visibleBooks.length===0?<p className="empty-note">La biblioteca está vacía. Los libros que agregues a D1 aparecerán aquí.</p>:visibleBooks.map(book=>{const match=filtered.includes(book),width=Math.round(Math.max(14,Math.min(88,16+book.pages*.1))),titleSize=Math.round(Math.max(8,Math.min(15,width/(Math.sqrt(book.title.length)*1.45)))*10)/10;return <div key={book.id} className="book-shell" style={{width}} onMouseEnter={()=>setSelected(book)}><button className={`book ${match?"match":"dim"} ${book.rightsStatus!=="rights_reserved"&&book.available<1?"borrowed":""}`} style={{width:"100%",background:book.color}} onClick={()=>{setSelected(book);setModal("detail")}}>{book.pages>=30&&<span style={{fontSize:titleSize}}>{book.title}</span>}{book.rightsStatus==="rights_reserved"?<i>↗</i>:book.available<1&&<i>•</i>}<div className="hover-cover" style={{background:book.cover}}><b>{book.title}</b><small>{book.author}</small></div></button>{currentUser?.role==="admin"&&<div className="book-admin-actions"><button type="button" title="Editar libro" aria-label={`Editar ${book.title}`} onClick={e=>{e.stopPropagation();setEditingBook(book)}}>✎</button><button type="button" title="Eliminar libro" aria-label={`Eliminar ${book.title}`} onClick={e=>{e.stopPropagation();deleteBook(book)}}><span className="trash-glyph" aria-hidden="true"/></button></div>}</div>})}</div><div className="wood"/></div><div className="shelf-pagination" aria-label="Cambiar de estante"><button disabled={shelfPage===0} onClick={()=>setShelfPage(page=>Math.max(0,page-1))} aria-label="Estante anterior">←</button><span>{shelfPage+1} / {shelfCount}</span><button disabled={shelfPage>=shelfCount-1} onClick={()=>setShelfPage(page=>Math.min(shelfCount-1,page+1))} aria-label="Estante siguiente">→</button></div></section>
      <aside className="right-panel">{selected?<><p className="eyebrow">LIBRO SELECCIONADO</p><div className="mini-cover" style={{background:selected.cover}}><span>{selected.title}</span></div><p className={`status ${selected.rightsStatus==="rights_reserved"&&!selected.reservedInternalAccess?"yes":selected.available?"yes":"no"}`}>{selected.rightsStatus==="rights_reserved"&&!selected.reservedInternalAccess?"Lectura en fuente externa":selected.available?`${selected.available} ${selected.available===1?"ejemplar disponible":"ejemplares disponibles"}`:"En préstamo"}</p><h2>{selected.title}</h2><p>{selected.author} · {selected.year}</p><div className="rating"><Stars rating={selected.rating}/><b>{selected.rating}</b></div><p className="synopsis">{selected.synopsis}</p><div className="facts"><span><b>{selected.pages}</b> páginas</span><span><b>{selected.type}</b> tipo</span></div>{selected.rightsStatus==="rights_reserved"?<div className="reserved-reading-actions"><p>Esta obra no suma páginas leídas. Puedes leerla en su fuente legal y marcarla como leída para habilitar reflexiones en el foro.</p>{selected.rightsSourceUrl&&<button className="primary wide" type="button" onClick={()=>window.open(selected.rightsSourceUrl,"_blank","noopener,noreferrer")}>Leer en la fuente oficial ↗</button>}<button className="secondary wide" type="button" disabled={reservedReadBusy} onClick={()=>markReservedRead(selected)}>{selected.markedRead?"✓ Marcado como leído":"Marcar como leído"}</button>{selected.reservedInternalAccess&&<button className="secondary wide" type="button" onClick={takeBook} disabled={!selected.available}>{!selected.available?"No disponible":"Leer desde Biblioteca Jornadas"}</button>}</div>:<button className="primary wide" onClick={takeBook} disabled={!selected.available}>{!selected.available?"No disponible":!currentUser?"Regístrate para tomarlo":!currentUser.emailVerified?"Verifica tu correo":currentUser.approvalStatus==="pending"?"Cuenta pendiente":currentUser.approvalStatus==="rejected"?"Registro rechazado":"Tomar este libro"}</button>}{isModerator&&isApproved&&selected.hasInternalContent&&<button className="inspection-button wide" type="button" onClick={()=>inspectBook(selected)}>Inspeccionar sin préstamo</button>}</>:<><p className="eyebrow">BIBLIOTECA VACÍA</p><h2>Aún no hay libros</h2><p className="synopsis">Cuando el administrador agregue el primer libro, aparecerá aquí.</p></>}</aside>
    </div>}

    {view==="comunidad"&&<section className="page community"><div className="page-heading"><p className="eyebrow">ZONA DE LECTORES</p><h1>Una comunidad entre páginas</h1><p>Descubre qué se está leyendo ahora, celebra el avance de otros lectores y encuentra tu próxima conversación.</p></div><div className="pulse"><span className="live-dot"/><b>{people.length} {people.length===1?"lector registrado":"lectores registrados"}</b><span>·</span><span>{people.reduce((total,p)=>total+p.pages,0).toLocaleString("es-MX")} páginas acumuladas</span></div><div className="people-grid">{people.length===0?<p className="empty-note">Todavía no hay lectores registrados.</p>:people.map((p,i)=><article key={p.id} className="person"><div className="person-top"><span className="place">#{i+1}</span><span>{p.role==="admin"?"Admin":p.role==="advisor"?"Asesor":"Lector"}</span></div><Avatar name={p.name} color={p.color} src={p.photoUrl}/><h3>{p.name}</h3><p><b>{p.pages.toLocaleString("es-MX")}</b> páginas leídas</p><div className="reading-now"><span style={{background:p.now?books.find(b=>b.title===p.now)?.cover:"#ddd"}}/><div><small>LEYENDO AHORA</small><b>{p.now||"Sin lectura activa"}</b></div></div><div className="reactions">{["❤️","✨","📚"].map(emojiValue=>{const key=`${p.name}-${emojiValue}`,active=profileReactions.includes(key),persisted=reactionCounts[reactionKey("profile",p.name,emojiValue)]||0,busy=reactionBusy.includes(reactionKey("profile",p.name,emojiValue));return <button key={key} className={active?"active":""} aria-pressed={active} disabled={busy} onClick={()=>toggleProfileReaction(p.name,emojiValue)}>{emojiValue} {persisted}</button>})}</div></article>)}</div></section>}

    {view==="notificaciones"&&currentUser&&<NotificationsPanel onUnreadChange={setNotificationUnread}/>}
    {view==="gestion"&&currentUser&&isApproved&&isModerator&&<ManagementPanel role={currentUser.role as "admin"|"advisor"} onCatalogChanged={()=>setCatalogNonce(value=>value+1)}/>}

    {view==="foro"&&<section className="page forum"><div className="forum-hero"><div><p className="eyebrow">EL FORO</p><h1>Ideas que siguen creciendo</h1><p>Publica reflexiones sobre los libros de tu historial y participa en cualquier conversación.</p></div><button className="primary" onClick={()=>document.getElementById("composer")?.focus()}>Escribir una reflexión</button></div><div className="forum-layout"><div className="feed"><div className="composer"><Avatar name={currentName} color="#cf915f" src={currentUser?.photoUrl}/><div className="composer-body"><textarea id="composer" value={draft} onChange={e=>setDraft(e.target.value)} placeholder="¿Qué idea se quedó contigo después de leer?"/><div><select value={postBook} onChange={e=>setPostBook(e.target.value)} aria-label="Libro leído relacionado" disabled={readBooks.length===0}>{readBooks.length===0?<option value="">Sin libros disponibles</option>:readBooks.map(b=><option key={b.id}>{b.title}</option>)}</select><small className="history-note">En obras con derechos reservados, primero márcalas como leídas.</small><button className="primary" onClick={publish} disabled={publishing||!currentUser||!isApproved||!postBook}>{publishing?"Publicando…":!currentUser?"Inicia sesión para publicar":!isApproved?"Cuenta pendiente":postBook?"Publicar":"No hay libros para publicar"}</button></div></div></div>{posts.map(post=><article className="post" key={post.id}><div className="post-author"><Avatar name={post.user} color={post.color} src={post.photoUrl}/><div><b>{post.user}</b><span>sobre <strong>{post.book}</strong></span></div><time>{post.time}</time></div><p>{post.text}</p><div className="post-actions"><button aria-pressed={liked.includes(post.id)} className={liked.includes(post.id)?"liked":""} disabled={reactionBusy.includes(reactionKey("post",post.id,"❤️"))} onClick={()=>toggleReaction("post",post.id,"❤️")}>{liked.includes(post.id)?"♥":"♡"} {post.likes+(reactionCounts[reactionKey("post",post.id,"❤️")]||0)}</button><button onClick={()=>setRepliesOpen(repliesOpen===post.id?null:post.id)}>↩ {post.replies} respuestas</button><button onClick={()=>flash("Enlace copiado")}>↗ Compartir</button></div>{repliesOpen===post.id&&<div className="reply-box"><Avatar name={currentName} color="#cf915f" small src={currentUser?.photoUrl}/><input value={replyDrafts[post.id]||""} onChange={e=>setReplyDrafts(current=>({...current,[post.id]:e.target.value}))} onKeyDown={e=>{if(e.key==="Enter")submitReply(post.id)}} placeholder={`Responder a ${post.user}…`}/><button onClick={()=>submitReply(post.id)}>Responder</button></div>}</article>)}</div><aside className="forum-side"><h3>Lecturas que conversamos</h3>{books.slice(0,4).map((b,i)=><button key={b.id} onClick={()=>{setSelected(b);setView("biblioteca")}}><span style={{background:b.cover}}/><div><b>{b.title}</b><small>{posts.filter(post=>post.book===b.title).length} reflexiones</small></div></button>)}<div className="prompt-card"><span>Pregunta de la semana</span><p>¿Qué personaje te enseñó algo sobre ti?</p><button onClick={()=>document.getElementById("composer")?.focus()}>Responder en el foro →</button></div></aside></div></section>}

    {view==="subir"&&currentUser&&isApproved&&<section className="page upload-page"><button className="back" onClick={()=>setView("biblioteca")}>← Volver al estante</button><div className="upload-wrap"><div className="upload-copy"><p className="eyebrow">{isModerator?"SUMAR A LA COLECCIÓN":"PROPONER UN LIBRO"}</p><h1>{isModerator?"Todo libro nuevo abre una puerta":"Comparte una lectura con la comunidad"}</h1><p>{isModerator?"Sube el archivo y documenta también la situación de derechos. Los materiales que queden “por revisar” se guardan ocultos hasta resolverla.":"Sube el archivo y completa la ficha, incluida la situación de derechos. La propuesta quedará pendiente hasta que un administrador o asesor la revise."}</p><blockquote>“Una biblioteca no se hace; crece.”<span>— Augustine Birrell</span></blockquote></div><form className="book-form" onSubmit={submitBook}><div className="drop"><span>＋</span><b>{uploadRightsStatus==="rights_reserved"?"Archivo interno opcional":"Archivo completo del libro"}</b><small>{uploadRightsStatus==="rights_reserved"?"Para derechos reservados puedes dejarlo vacío y enlazar la fuente legal. Si subes un archivo, quedará inaccesible hasta que un admin habilite lectura interna.":"PDF o TXT · máximo 25 MB · reconstrucción automática"}</small><input name="file" required={uploadRightsStatus!=="rights_reserved"} type="file" accept=".pdf,.txt,text/plain,application/pdf" aria-label="Archivo del libro" onChange={e=>handleBookFile(e.target.files?.[0])}/>{fileInfo&&<em className={detectedPages>0?"file-ready":"file-error"}>{fileInfo}</em>}</div><div className="field full"><label>Título</label><input name="title" required maxLength={180} placeholder="Ej. El jardín secreto"/></div><div className="field"><label>Autor</label><input name="author" required maxLength={140} placeholder="Nombre del autor"/></div><div className="field"><label>Año</label><input name="year" required type="number" min="1" max={new Date().getFullYear()+1} placeholder="2024"/></div><div className="field"><label>Número de páginas</label><input name="pages" required type="number" min="1" max="20000" value={manualPages} readOnly={detectedPages>0} onChange={e=>setManualPages(e.target.value)} placeholder={uploadRightsStatus==="rights_reserved"?"Indica las páginas de la edición":"Se calcula al subir el archivo"}/><small>{detectedPages>0?"Páginas reconstruidas desde el documento":uploadRightsStatus==="rights_reserved"?"Dato bibliográfico; no contará en estadísticas":"Selecciona primero el archivo"}</small></div><div className="field"><label>Tipo</label><select name="type" required>{BOOK_TYPES.map(item=><option key={item} value={item}>{item}</option>)}</select></div><div className="field"><label>Ejemplares disponibles</label><input name="copies" required type="number" defaultValue="1" min="1" max="1000"/></div><div className="field full"><label>Sinopsis</label><textarea name="synopsis" required maxLength={3000} placeholder="Cuéntanos de qué trata, sin revelar demasiado…"/></div><div className="rights-section field full"><div><p className="eyebrow">DERECHOS DE AUTOR</p><h3>¿Cuál es la situación de derechos de esta obra?</h3><small>Si no estás seguro, elige “Situación por revisar”. En “Derechos reservados” la ficha se publica con un enlace a la fuente indicada; un archivo interno, si existe, queda bloqueado hasta una decisión explícita del administrador.</small></div><div className="rights-grid"><label>Situación<select name="rightsStatus" required value={uploadRightsStatus} onChange={e=>setUploadRightsStatus(e.target.value)}>{RIGHTS_OPTIONS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Titular o responsable<input name="rightsHolder" maxLength={180} placeholder="Autor, editorial, institución…"/></label><label>Fuente o licencia<input name="rightsSourceUrl" type="url" maxLength={1000} placeholder="https://…"/></label><label>Quién concedió el permiso<input name="rightsPermissionBy" maxLength={180} placeholder="Nombre y cargo, si aplica"/></label><label className="full">Notas de derechos<textarea name="rightsNotes" maxLength={2000} placeholder="Licencia CC concreta, fecha del permiso, fundamento de dominio público, condiciones de reproducción…"/></label><label className="full rights-evidence">Evidencia privada <small>Opcional · PDF, TXT, JPG, PNG o WEBP · máx. 10 MB</small><input name="rightsEvidence" type="file" accept=".pdf,.txt,.jpg,.jpeg,.png,.webp,application/pdf,text/plain,image/jpeg,image/png,image/webp"/></label></div></div><button className="primary submit" disabled={bookUploading||(uploadRightsStatus!=="rights_reserved"&&(detectedPages<1||!bookPackage))}>{bookUploading?(isModerator?"Guardando libro…":"Enviando propuesta…"):uploadRightsStatus!=="rights_reserved"&&detectedPages<1?"Reconstruyendo libro…":isModerator?"Guardar con control de derechos":"Enviar propuesta para revisión"}</button></form></div></section>}

    {view==="lector"&&readerBook&&<section className={`reader reader-book-mode ${readerMode==="continuous"?"reader-continuous-mode":""}`}><div className="reader-bar"><button onClick={()=>inspectionMode?endInspection():setView("biblioteca")}>← {inspectionMode?"Salir de inspección":"Cerrar lector"}</button><div className="reader-title"><b>{readerBook.title}</b><span>{inspectionMode?"Inspección sin préstamo":readerMode==="continuous"?"Lectura continua":readerPage===0?"Portada":readerPage===backCoverPage?"Final":`Páginas ${readerPage}–${Math.min(readerTotalPages,readerPage+1)} de ${readerTotalPages}`}</span></div><div className="reader-toolbar-right"><div className="reader-mode-switch" aria-label="Modo de lectura"><button type="button" className={readerMode==="book"?"active":""} onClick={()=>switchReaderMode("book")}>Libro</button><button type="button" className={readerMode==="continuous"?"active":""} onClick={()=>switchReaderMode("continuous")}>Lectura</button></div>{readerMode==="book"?<div className="reader-tools"><small>Zoom</small><button type="button" onClick={()=>changeReaderZoom(-.1)} disabled={readerZoom<=.7} aria-label="Alejar">−</button><span>{Math.round(readerZoom*100)}%</span><button type="button" onClick={()=>changeReaderZoom(.1)} disabled={readerZoom>=2} aria-label="Acercar">+</button></div>:<div className="reader-tools continuous-text-tools"><small>Texto</small><button type="button" onClick={()=>changeContinuousFont(-2)} disabled={continuousFontSize<=18} aria-label="Reducir letra">A−</button><span>{continuousFontSize}px</span><button type="button" onClick={()=>changeContinuousFont(2)} disabled={continuousFontSize>=42} aria-label="Aumentar letra">A+</button></div>}</div></div>{readerMode==="continuous"?<><div className="reader-continuous-progress" aria-hidden="true"><span style={{width:`${readerBook.progress??0}%`}}/></div><div ref={readerContinuousRef} className="reader-continuous-stage" onScroll={handleContinuousScroll} onWheel={handleContinuousWheel}>{readerLoading?<div className="reader-message">Preparando lectura continua…</div>:readerError?<div className="reader-message error">{readerError}</div>:<article className="reader-continuous-document" style={{fontSize:`${continuousFontSize}px`}}><header className="reader-continuous-cover"><p>LECTURA CONTINUA</p><h1>{readerBook.title}</h1><span>{readerBook.author}</span></header>{readerPages.map((page,index)=><section className="reader-continuous-section" key={index} data-page={index+1}>{page.artwork&&<img src={page.artwork} alt="Ilustración del libro"/>}{page.heading&&<h2>{page.heading}</h2>}{page.paragraphs.map((paragraph,paragraphIndex)=><p key={paragraphIndex}>{paragraph}</p>)}<small className="reader-continuous-page-marker">Página original {index+1}</small></section>)}<footer className="reader-continuous-finish"><span>FIN</span><h2>{readerBook.title}</h2><p>{inspectionMode?"Fin de la inspección. No se registró progreso ni estadísticas.":"Has llegado al final de la lectura."}</p><button className="primary" onClick={inspectionMode?endInspection:openReturnModal}>{inspectionMode?"Salir de inspección":"Devolver libro"}</button></footer></article>}</div><div className="reader-continuous-footer"><span>{Math.round(readerBook.progress??0)}% {inspectionMode?"inspeccionado":"leído"}</span><small>{inspectionMode?"Modo inspección: este avance no se guarda y no suma páginas.":"Desplázate hacia abajo · Ctrl + rueda o A− / A+ cambia el tamaño del texto"}</small></div></>:<><div className="reader-stage book-stage" onPointerDown={handleReaderPointerDown} onPointerMove={handleReaderPointerMove} onPointerUp={handleReaderPointerEnd} onPointerCancel={handleReaderPointerEnd} onWheel={handleReaderWheel}>{readerLoading?<div className="reader-message">Reconstruyendo la lectura…</div>:readerError?<div className="reader-message error">{readerError}</div>:<><button className="reader-side-nav prev" onClick={previousReaderSpread} disabled={readerAnimating||readerClosing||readerReturnVisible||readerPage===0} aria-label="Páginas anteriores">‹</button><div className="reader-book-scene"><div className="reader-zoom-shell" style={{transform:`scale(${readerZoom})`}}>{readerPage===0?<div className={`reader-cover-shell ${readerAnimating&&readerPendingPage===1?"opening":"closed"}`}><div className="reader-cover-underlay" aria-hidden="true"><ReaderSpread pages={readerPages} start={1}/></div><div className="reader-cover-pageblock" aria-hidden="true"/><div className="reader-cover-hinge"><button className="reader-cover" onClick={nextReaderSpread} disabled={readerAnimating} aria-label="Abrir libro"><span className="reader-cover-front" style={{background:readerBook.cover}}>{readerPages[0]?.artwork&&<img className="reader-cover-art" src={readerPages[0].artwork} alt="Portada recuperada del documento"/>}<span className="reader-cover-kicker">Biblioteca MJVC Mérida</span><strong>{readerBook.title}</strong><small>{readerBook.author}</small><i>Haz clic para abrir</i></span><span className="reader-cover-back" aria-hidden="true"/></button></div></div>:readerAnimating&&readerPage===1&&readerPendingPage===0?<div className="reader-cover-shell closing"><div className="reader-cover-underlay" aria-hidden="true"><ReaderSpread pages={readerPages} start={1}/></div><div className="reader-cover-pageblock" aria-hidden="true"/><div className="reader-cover-hinge"><button className="reader-cover" type="button" disabled aria-label="Cerrando libro"><span className="reader-cover-front" style={{background:readerBook.cover}}>{readerPages[0]?.artwork&&<img className="reader-cover-art" src={readerPages[0].artwork} alt="Portada recuperada del documento"/>}<span className="reader-cover-kicker">Biblioteca MJVC Mérida</span><strong>{readerBook.title}</strong><small>{readerBook.author}</small></span><span className="reader-cover-back" aria-hidden="true"/></button></div></div>:readerPage===backCoverPage?<div className={`reader-finish-layout ${readerReturnVisible?"return-visible":""}`}><div className="reader-closed-back-cover" style={{background:readerBook.cover}}><span>Biblioteca MJVC Mérida</span><strong>Fin</strong><b>{readerBook.title}</b><small>{readerBook.author}</small></div>{inspectionMode?<div className="reader-inline-return inspection-finish"><div className="reader-inline-return-head"><p className="eyebrow">MODO INSPECCIÓN</p><h2>Revisión terminada</h2><p>Puedes salir cuando quieras. No se creó préstamo, no se guardó progreso y no se sumarán páginas leídas.</p></div><div className="reader-inline-return-actions"><button type="button" className="secondary" onClick={reopenLastSpread}>Volver a las últimas páginas</button><button type="button" className="primary" onClick={endInspection}>Salir de inspección</button></div></div>:<form className="reader-inline-return" onSubmit={returnBook}><div className="reader-inline-return-head"><p className="eyebrow">ANTES DE DEVOLVERLO</p><h2>Deja una huella para quien sigue</h2><p>Tu calificación ayuda a la comunidad. Puedes responder hasta 3 preguntas y proponer otra de forma opcional.</p></div><div className="reader-inline-return-body">{returnQuestionsLoading?<p className="question-empty">Cargando preguntas…</p>:returnQuestions.map((question,index)=><label key={question.id}><span>{index+1}. {question.body}</span><textarea name={`answer-${question.id}`} required placeholder="Escribe tu respuesta…"/></label>)}{returnExtraQuestions>0&&<p className="question-extra">Hay {returnExtraQuestions} preguntas adicionales; no necesitas responderlas ahora.</p>}<label>Pregunta para un futuro lector <small className="optional-note">Opcional</small><textarea name="newQuestion" maxLength={500} placeholder="Puedes dejarlo vacío."/></label><label>Tu calificación<Stars rating={returnRating} onSelect={setReturnRating}/><small className="rating-help">{returnRating?`${returnRating} de 5 estrellas`:"Selecciona de 1 a 5 estrellas"}</small></label></div><div className="reader-inline-return-actions"><button type="button" className="secondary" onClick={reopenLastSpread}>Volver a las últimas páginas</button><button type="button" className="secondary abandon-reading" onClick={abandonBook}>Devolver sin terminar</button><button className="primary" disabled={returnQuestionsLoading}>Completar devolución</button></div><small className="abandon-reading-note">“Devolver sin terminar” no guarda respuestas ni calificación y elimina este préstamo del conteo de páginas leídas.</small></form>}</div>:readerAnimating&&readerPendingPage===backCoverPage?<ReaderFinalCloseTransition pages={readerPages} start={readerPage} cover={readerBook.cover} title={readerBook.title} author={readerBook.author}/>:readerAnimating&&readerPendingPage&&readerPendingPage>0?<AnimatedReaderSpread pages={readerPages} currentStart={readerPage} targetStart={readerPendingPage} direction={readerTurn}/>:<ReaderSpread pages={readerPages} start={readerPage}/>}</div></div><button className="reader-side-nav next" onClick={nextReaderSpread} disabled={readerAnimating||readerClosing||readerReturnVisible||readerTotalPages<1||readerPage===backCoverPage} aria-label={readerPage===0?"Abrir libro":readerPage===lastSpreadStart?"Cerrar libro":"Páginas siguientes"}>›</button><div className="reader-preload-cache" aria-hidden="true">{preloadPrevStart&&preloadPrevStart>0&&preloadPrevStart<=lastSpreadStart&&<ReaderSpread pages={readerPages} start={preloadPrevStart} className="preloaded-prev"/>}{preloadNextStart&&preloadNextStart>0&&preloadNextStart<=lastSpreadStart&&<ReaderSpread pages={readerPages} start={preloadNextStart} className="preloaded-next"/>}</div></>}</div><div className="reader-book-footer"><span>{readerPage===0?"Portada":readerPage===backCoverPage?(inspectionMode?"Inspección terminada":"Libro terminado"):`${inspectionMode?"Inspeccionando":"Leyendo"} ${Math.min(readerTotalPages,readerPage+1)} de ${readerTotalPages}`}</span><small>{inspectionMode?"Modo inspección · sin préstamo, preguntas, calificación ni páginas leídas":readerPage===backCoverPage?(readerReturnVisible?"Completa la devolución o vuelve a las últimas páginas":"Cerrando el libro…"):readerPage===lastSpreadStart?"La siguiente vuelta cierra el libro":"Se precargan las páginas vecinas · Flechas del teclado · Pellizca o usa el trackpad para hacer zoom"}</small></div></>}</section>}

    {modal==="detail"&&selected&&<div className="modal-back" onClick={()=>setModal(null)}><div className="book-modal" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setModal(null)}>×</button><div className="modal-cover" style={{background:selected.cover}}><span>{selected.title}</span><small>{selected.author}</small></div><div className="modal-copy"><p className="eyebrow">{selected.type.toUpperCase()}</p><h2>{selected.title}</h2><p className="by">{selected.author} · {selected.year}</p><div className="rating"><Stars rating={selected.rating}/><b>{selected.rating}</b><span>({selected.reads} {selected.reads===1?"lectura":"lecturas"})</span></div><p>{selected.synopsis}</p><div className="meta-row"><span><b>{selected.pages}</b> páginas</span><span><b>{selected.type}</b> tipo</span></div><div className="rights-public-card"><small>Situación de derechos</small><b>{rightsLabel(selected.rightsStatus)}</b>{selected.rightsHolder&&<span>{selected.rightsHolder}</span>}{selected.rightsStatus==="rights_reserved"&&<span>{selected.reservedInternalAccess?"Lectura interna habilitada por administración · no suma páginas":"Lectura desde la fuente legal · no suma páginas"}</span>}{selected.rightsSourceUrl&&<a href={selected.rightsSourceUrl} target="_blank" rel="noreferrer">Consultar fuente ↗</a>}</div>{selected.rightsStatus==="rights_reserved"?<div className="reserved-reading-actions modal-reserved-actions"><p>Las obras con derechos reservados no generan páginas leídas. Cuando termines, puedes marcarla como leída para que aparezca entre tus opciones de reflexión.</p>{selected.rightsSourceUrl&&<button className="primary wide" type="button" onClick={()=>window.open(selected.rightsSourceUrl,"_blank","noopener,noreferrer")}>Leer directamente en la fuente ↗</button>}<button className="secondary wide" type="button" disabled={reservedReadBusy} onClick={()=>markReservedRead(selected)}>{selected.markedRead?"✓ Marcado como leído":"Marcar como leído"}</button>{selected.reservedInternalAccess&&<button className="secondary wide" type="button" disabled={!selected.available} onClick={takeBook}>{!selected.available?"Lectura interna ocupada":"Leer desde Biblioteca Jornadas"}</button>}</div>:<button className="primary wide" disabled={!selected.available} onClick={takeBook}>{!selected.available?"En préstamo":!currentUser?"Regístrate para tomarlo":!currentUser.emailVerified?"Verifica tu correo":currentUser.approvalStatus==="pending"?"Cuenta pendiente":currentUser.approvalStatus==="rejected"?"Registro rechazado":"Tomar este libro"}</button>}{isModerator&&isApproved&&selected.hasInternalContent&&<button className="inspection-button wide" type="button" onClick={()=>inspectBook(selected)}>Inspeccionar sin préstamo</button>}<button className="copyright-report-link" type="button" onClick={()=>{if(!currentUser||!isApproved){flash("Inicia sesión con una cuenta aprobada para reportar un problema de derechos.");return}setModal("copyright")}}>Reportar problema de derechos de autor</button></div></div></div>}
    {modal==="copyright"&&selected&&currentUser&&<div className="modal-back" onClick={()=>!copyrightSubmitting&&setModal(null)}><form className="copyright-modal" onClick={e=>e.stopPropagation()} onSubmit={submitCopyrightReport}><button type="button" className="close" disabled={copyrightSubmitting} onClick={()=>setModal(null)}>×</button><p className="eyebrow">DERECHOS DE AUTOR</p><h2>Reportar “{selected.title}”</h2><p>Usa este formulario si eres titular, autor, editorial, representante o conoces un problema concreto con los derechos de esta obra. La administración puede ocultarla mientras se revisa.</p><label>Nombre del titular o reclamante<input name="claimantName" required maxLength={160} defaultValue={currentUser.name}/></label><label>Correo del reclamante<input name="claimantEmail" required type="email" maxLength={254} defaultValue={currentUser.email}/></label><label>Relación con la obra<select name="relationship" required defaultValue="Titular o autor"><option>Titular o autor</option><option>Representante del titular</option><option>Editorial</option><option>Persona autorizada</option><option>Otro</option></select></label><label>Fuente o evidencia en línea <small>opcional</small><input name="evidenceUrl" type="url" maxLength={1000} placeholder="https://…"/></label><label>Describe el problema<textarea name="details" required maxLength={3000} placeholder="Explica qué derechos consideras afectados y qué acción solicitas."/></label><p className="copyright-policy-note">También puedes consultar nuestra <a href="/derechos-de-autor" target="_blank" rel="noreferrer">política de materiales y retirada</a>.</p><button className="primary wide" disabled={copyrightSubmitting}>{copyrightSubmitting?"Enviando…":"Enviar reclamación"}</button></form></div>}
    {modal==="return"&&owned&&<div className="modal-back"><form className="return-modal" onSubmit={returnBook}><button type="button" className="close" onClick={()=>setModal(null)}>×</button><p className="eyebrow">ANTES DE DEVOLVERLO</p><h2>Deja una huella para quien sigue</h2><p>Tu calificación ayuda a la comunidad. También puedes responder preguntas de lectores anteriores y dejar una nueva.</p><div className="reader-questions"><div className="question-heading"><b>Preguntas de lectores anteriores</b><small>Solo pedimos responder un máximo de 3 preguntas. Si existen más, las demás no son obligatorias y quedarán para futuras lecturas.</small></div>{returnQuestionsLoading?<p className="question-empty">Cargando preguntas…</p>:returnQuestions.length===0?<p className="question-empty">Todavía no hay preguntas pendientes para ti.</p>:returnQuestions.map((question,index)=><label key={question.id}><span>{index+1}. {question.body}</span><small>Propuesta por {question.user}</small><textarea name={`answer-${question.id}`} required placeholder="Escribe tu respuesta…"/></label>)}{returnExtraQuestions>0&&<p className="question-extra">Hay {returnExtraQuestions} {returnExtraQuestions===1?"pregunta adicional":"preguntas adicionales"}; no necesitas responderlas ahora.</p>}</div><label>Propón una pregunta para un futuro lector <small className="optional-note">Opcional</small><textarea name="newQuestion" maxLength={500} placeholder="¿Qué te gustaría preguntarles? Puedes dejarlo vacío."/></label><label>¿Cómo calificas esta lectura?<Stars rating={returnRating} onSelect={setReturnRating}/><small className="rating-help">{returnRating?`${returnRating} de 5 estrellas`:"Selecciona de 1 a 5 estrellas"}</small></label><div className="return-choice-actions"><button type="button" className="secondary wide abandon-reading" onClick={abandonBook}>Devolver sin terminar</button><button className="primary wide" disabled={returnQuestionsLoading}>Completar devolución</button></div><small className="abandon-reading-note">Si lo devuelves sin terminar, no pediremos respuestas ni calificación y todo el avance de este préstamo dejará de contar como páginas leídas.</small></form></div>}
    {editingBook&&<div className="modal-back" onClick={()=>!bookAdminBusy&&setEditingBook(null)}><form className="profile-modal book-edit-modal" onClick={e=>e.stopPropagation()} onSubmit={saveBookEdit}><button type="button" className="close" disabled={bookAdminBusy} onClick={()=>setEditingBook(null)}>×</button><p className="eyebrow">ADMINISTRAR LIBRO</p><h2>Editar ficha</h2><p className="edit-pages-note">{editingBook.pages} páginas · calculadas desde el archivo</p><label>Título<input name="title" required maxLength={180} defaultValue={editingBook.title}/></label><label>Autor<input name="author" required maxLength={140} defaultValue={editingBook.author}/></label><div className="edit-book-row"><label>Año<input name="year" required type="number" min="1" max={new Date().getFullYear()+1} defaultValue={editingBook.year}/></label><label>Ejemplares<input name="copies" required type="number" min="1" max="1000" defaultValue={editingBook.copies}/></label></div><label>Tipo<select name="type" defaultValue={editingBook.type}>{BOOK_TYPES.map(item=><option key={item} value={item}>{item}</option>)}</select></label><label>Sinopsis<textarea name="synopsis" required maxLength={3000} defaultValue={editingBook.synopsis}/></label><div className="edit-rights-note"><b>Derechos: {rightsLabel(editingBook.rightsStatus)}</b><span>La situación jurídica y publicación se administran desde Gestión → Derechos.</span></div><button className="primary wide" disabled={bookAdminBusy}>{bookAdminBusy?"Guardando…":"Guardar cambios"}</button><button type="button" className="danger-link" disabled={bookAdminBusy} onClick={()=>deleteBook(editingBook)}>Eliminar libro</button></form></div>}
    <footer className="site-footer"><div><b>Biblioteca Jornadas</b><span>Una herramienta digital para la comunidad de Jornadas de Vida Cristiana Mérida.</span></div><nav><a href="/privacidad">Política de privacidad</a><a href="/terminos">Términos de uso</a><a href="/derechos-de-autor">Derechos de autor</a><a href="mailto:biblioteca.jornadas@gmail.com">Contacto</a></nav></footer>

    {modal==="profile"&&currentUser&&<div className="modal-back" onClick={()=>setModal(null)}><form className="profile-modal" onClick={e=>e.stopPropagation()} onSubmit={saveProfile}><button type="button" className="close" onClick={()=>setModal(null)}>×</button><p className="eyebrow">MI PERFIL</p><div className="profile-photo-wrap"><Avatar name={currentName} color="#cf915f" src={currentUser.photoUrl}/><label className="photo-button">{photoUploading?"Subiendo foto…":"Cambiar foto"}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={photoUploading} onChange={e=>{uploadProfilePhoto(e.target.files?.[0]);e.currentTarget.value=""}}/></label><small>JPG, PNG o WEBP · máximo 4 MB</small></div><h2>{currentUser.name}</h2><p className="profile-role">{currentUser.role==="admin"?"Administrador":currentUser.role==="advisor"?"Asesor":"Lector"} · {!currentUser.emailVerified?"Correo sin verificar":currentUser.approvalStatus==="approved"?"Cuenta aprobada":currentUser.approvalStatus==="pending"?"Pendiente":"Rechazada"}</p><label>Correo electrónico<input value={currentUser.email} readOnly/></label><label>Sobre mí<textarea name="description" maxLength={320} defaultValue={currentUser.description} placeholder="Cuéntale a la comunidad un poco sobre ti…"/></label><div className="stats"><span><b>{currentUser.pagesRead.toLocaleString("es-MX")}</b> páginas leídas</span><span><b>{currentUser.role==="admin"?"Administrador":currentUser.role==="advisor"?"Asesor":"Lector"}</b> rol</span></div><button className="primary wide" disabled={profileSaving}>{profileSaving?"Guardando…":"Guardar perfil"}</button></form></div>}
    {authView&&<div className="modal-back" onClick={()=>setAuthView(null)}><form className="auth-modal" onClick={e=>e.stopPropagation()} onSubmit={submitAuth}><button type="button" className="close" onClick={()=>setAuthView(null)}>×</button><div className="auth-mark">B</div>{authView==="login"&&<><p className="eyebrow">ACCESO DE LECTORES</p><h2>Iniciar sesión</h2><p>Continúa con tus préstamos, lecturas y conversaciones.</p><label>Correo electrónico<input name="email" required type="email" placeholder="nombre@correo.com" autoComplete="email"/></label><label>Contraseña<input name="password" required type="password" placeholder="Tu contraseña" autoComplete="current-password"/></label><button className="forgot" type="button" onClick={()=>setAuthView("recover")}>Olvidé mi contraseña</button><button className="primary wide" disabled={authSubmitting}>{authSubmitting?"Entrando…":"Iniciar sesión"}</button><p className="auth-switch">¿Aún no tienes cuenta? <button type="button" onClick={()=>setAuthView("signup")}>Regístrate</button></p></>}{authView==="signup"&&<><p className="eyebrow">NUEVA CUENTA</p><h2>Crear una cuenta</h2><p>Completa tus datos. Primero te enviaremos un correo con un enlace de verificación; solo después de abrirlo llegará tu solicitud al administrador y a los asesores.</p><label>Nombre<input name="name" required minLength={2} maxLength={80} placeholder="Tu nombre" autoComplete="name"/></label><label>Correo electrónico<input name="email" required type="email" placeholder="nombre@correo.com" autoComplete="email"/></label><label>Contraseña<input name="password" required type="password" minLength={8} maxLength={128} placeholder="Mínimo 8 caracteres" autoComplete="new-password"/></label><label>Confirmar contraseña<input name="confirmPassword" required type="password" minLength={8} maxLength={128} placeholder="Repite tu contraseña" autoComplete="new-password"/></label><label>Código de administrador <small>opcional, solo para la configuración inicial</small><input name="adminCode" type="password" placeholder="Déjalo vacío si eres lector" autoComplete="off"/></label><button className="primary wide" disabled={authSubmitting}>{authSubmitting?"Enviando…":"Crear cuenta y verificar correo"}</button><p className="auth-switch">¿Ya tienes cuenta? <button type="button" onClick={()=>setAuthView("login")}>Inicia sesión</button></p></>}{authView==="recover"&&<><p className="eyebrow">RECUPERAR ACCESO</p><h2>Recuperación de contraseña</h2><div className="recovery-success"><b>Recuperación por correo aún no configurada</b><p>Por ahora solicita al administrador que restablezca tu acceso. No enviaremos un correo ficticio.</p><button type="button" className="secondary wide" onClick={()=>setAuthView("login")}>Volver a iniciar sesión</button></div></>}</form></div>}
